...
import { fetchSeminars, fetchAnnouncements, createAnnouncement } from "../../data/api";
...
export function UploadInformasi() {
  ...
  const [form, setForm] = useState({ seminar: "", type: "Zoom Link", title: "", content: "", hasFile: false });
  const [submitted, setSubmitted] = useState(false);

  useEffect(() => {
    fetchSeminars().then(setSeminarList).catch(console.error);
    fetchAnnouncements().then(setRecentUploads).catch(console.error);
  }, []);
          ...
          <div className="bg-white border border-[#E2E8F0] rounded-2xl overflow-hidden" style={{ boxShadow: "0px 4px 20px rgba(0,82,204,0.05)" }}>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              <div>
                ...
                <select value={form.seminar} onChange={e => handleChange("seminar", e.target.value)} required className="w-full px-4 py-2.5 text-sm border border-[#E2E8F0] rounded-xl bg-white outline-none focus:border-[#0052CC]">
                  <option value="">-- Pilih Seminar --</option>
                  {seminarList.map(s => <option key={s.id} value={s.id}>{s.title}</option>)}
                </select>
              </div>
              <div>
              ...
              <div>
                ...
                <input type="text" value={form.title} onChange={e => handleChange("title", e.target.value)} placeholder="Judul informasi..." required className="w-full px-4 py-2.5 text-sm border border-[#E2E8F0] rounded-xl bg-white outline-none focus:border-[#0052CC]" />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#374151] mb-1.5">{form.type === "Zoom Link" ? "URL Zoom / Meeting Link" : "Konten / URL"}</label>
                <textarea value={form.content} onChange={e => handleChange("content", e.target.value)} rows={3} placeholder={form.type === "Zoom Link" ? "https://zoom.us/j/..." : "Tulis konten atau URL..."} className="w-full px-4 py-2.5 text-sm border border-[#E2E8F0] rounded-xl bg-white outline-none focus:border-[#0052CC] resize-none" />
              </div>
              <div>
                <label className="block text-sm font-medium text-[#374151] mb-1.5">Lampiran File (Opsional)</label>
                <div onClick={() => handleChange("hasFile", !form.hasFile)} className={`border-2 border-dashed rounded-xl p-5 text-center cursor-pointer transition-all ${form.hasFile ? "border-[#22C55E] bg-green-50" : "border-[#E2E8F0] hover:border-[#0052CC]/50"}`}>
                  {form.hasFile ? (
                    <div className="flex items-center justify-center gap-2 text-green-600">
                      <CheckCircle size={18} /><span className="text-sm font-medium">File terlampir</span>
                      <button type="button" onClick={() => handleChange("hasFile", false)}><X size={14} /></button>
                    </div>
                  ) : (
                    <><Upload size={24} className="mx-auto text-[#94A3B8] mb-2" /><p className="text-xs text-[#64748B]">Klik untuk upload file (PDF/DOCX/PPTX, maks. 10MB)</p></>
                  )}
                </div>
              </div>
              <div className="flex justify-end">
                <button type="submit" className="px-6 py-2.5 bg-[#22C55E] text-white text-sm font-medium rounded-xl hover:bg-[#16A34A] transition-colors flex items-center gap-2">
                ...
                return (
                  <div key={upload.id} className="flex items-start gap-3 p-3 bg-[#F8FAFC] rounded-xl">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-[#64748B] truncate">{upload.seminar?.title || "-"}</p>
